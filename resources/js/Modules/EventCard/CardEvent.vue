    ...
    <div class="flex flex-col justify-start border border-transparent">
      <div class="group relative aspect-a4 min-w-40 overflow-hidden rounded-md bg-base-300 shadow-md shadow-base-200 transition-all duration-200 hover:cursor-pointer hover:shadow-lg hover:shadow-neutral-500">
        <div class="size-full scale-90">
          ...
          <template v-else>
            <div class="absolute left-1/2 top-1/2 size-full -translate-x-1/2 -translate-y-1/2 cursor-pointer select-none transition-all delay-150 duration-200 group-hover:left-2/4 group-hover:top-1/4 group-hover:scale-75">
              <IconTypeMapper type="imageOff" class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 text-6xl" />
            </div>
          </template>
          <div class="absolute left-1 top-1 z-50 flex translate-y-1/2 flex-col items-center justify-center gap-0.5 opacity-0 transition-all delay-150 duration-200 group-hover:translate-y-0 group-hover:opacity-100">
            <div class="tooltip tooltip-right " data-tip="link">
              <Link :href="route('event.show', event.alias)" class="btn btn-square btn-neutral btn-sm">
                <IconTypeMapper type="link" class="size-6" />
              </Link>
            </div>
            <div class="tooltip tooltip-right" data-tip="Share">
              <button class="btn btn-square btn-neutral btn-sm" @click="emit('share')">
                <IconTypeMapper type="share" class="size-6" />
              </button>
            </div>
          </div>
          <div class="absolute right-1 top-1 z-50 flex translate-y-1/2 flex-col items-center justify-center gap-0.5 opacity-0 transition-all delay-150 duration-200 group-hover:translate-y-0 group-hover:opacity-100">
            <div class="tooltip tooltip-left" data-tip="Bookmark">
              <BtnSwapEventBookmark class="btn btn-square btn-neutral btn-sm" :event-id="event.alias" :check="event.auth_user?.is_bookmark" />
            </div>
            <div class="tooltip tooltip-left" data-tip="Good">
              <BtnSwapEventGood
