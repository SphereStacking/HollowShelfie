...
<template>
  <div class="flex flex-col overflow-hidden overflow-x-auto">
    <div class="flex flex-row">
      <div
        class="grid grid-rows-2  gap-0.5"
        :style="`grid-template-columns: repeat(${columnNum()}, minmax(2.0rem, 1fr))`">
        <template v-for="time in formattedDates() ">
          <!-- Horizontal lines -->
          <div class="row-start-1 text-left " style="grid-column:  span 96">
            <!-- 日にちを表示 -->
            <h2 class="sticky text-xl font-semibold ">
              {{ time }}
            </h2>
          </div>
          <div v-for="(hour, index) in 24" class="col-span-4 row-start-2 grid">
            <div class="badge badge-ghost  left-0 w-full  leading-5 ">
              {{ index }}:00
            </div>
          </div>
        </template>
    ...
    <div
      class="relative grid grid-rows-2 gap-0.5"
      :style="`grid-template-columns: repeat(${columnNum()}, minmax(2.0rem, 1fr))`">
      <!-- 現在時刻のバーを追加 -->
      <div
        :style="{
        ...
        }" class="absolute ml-2  h-full w-1 bg-red-500">
      </div>
      <template v-for="( task, index ) in events ">
        <div
          class="row-span-2 flex flex-col" :style="{
            gridColumn: calculateGridPosition(task.start_date, task.end_date)
          }
          ">
          <div class="w-full rounded-xl border bg-primary text-center text-primary-content" @click="logOutput(task)">
            {{ task.title }}
            <div class="mx-0.5 mb-1 flex flex-row justify-around">
              <div v-for="( item, timetableIndex ) in task.timetable " class="badge badge-ghost mx-0.5 w-full">
                {{ item.title }}
              </div>
            </div>
          </div>
